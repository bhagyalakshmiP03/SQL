...
#2.
SELECT FNAME , JOB, DOB
FROM EMP;

#3
select * FROM EMP;
#4
SELECT FNAME, SAL, (SAL*12) AS ANNUAL_SALARY
FROM EMP;

#5
SELECT FNAME, SAL, ((SAL*12)+50000) AS ANNUAL_SALARY
FROM EMP;

#6
SELECT FNAME,(SAL+5000) AS SALARY_WITH_INCENTIVE
FROM EMP;

#7
SELECT FNAME,(SAL + (SAL * 0.05)) AS SALARY_WITH_5PERCENT_INCENTIVE
FROM EMP;

#8
SELECT FNAME,(SAL - (SAL * 0.07)) AS SALARY_WITH_7PERCENT_DEDUCTION
FROM EMP;
...
#10
SELECT FNAME, SAL + SAL*0.1 , COMM - COMM*0.07
FROM EMP;

#11
SELECT FNAME, LNAME, JOB AS DESIGNATION, DNO AS TEAM
FROM EMP;
...
--->MULTIPLE_CONDITIONS USING_WHERE_CLAUSE
...
FROM EMP
WHERE SAL>40000 AND JOB="SALESMAN";
...
#26.
SELECT * FROM EMP
WHERE (JOB IN ("SALESMAN" , "DISPATCHER" , "ACCOUNTANT"))AND (DNO IN (110,111,112));
...
#28.
SELECT * FROM EMP
WHERE (JOB="MANAGER" OR JOB="DEVELOPER" OR JOB= "TESTER" ) AND (DNO != 111 AND DNO!= 112);
--OR_USING_MULTIVALUE_OPERATOR
SELECT * FROM EMP
WHERE JOB  IN ("MANAGER","DEVELOPER","TESTER") AND DNO NOT IN (111,112);
...
SELECT COUNT(*), DNO
FROM EMP
WHERE JOB IN ("SALESMAN","MANAGER", "DISPATCHER", "ACCOUNTANT") 
GROUP BY DNO
HAVING AVG(SAL)>32000
ORDER BY COUNT(*);
