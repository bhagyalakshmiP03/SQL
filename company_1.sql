...
FROM EMP;
#OR___USING_LEFT()_FUNCTION
SELECT LEFT(FNAME, 3)
FROM EMP;


#99.
SELECT substr(FNAME, -4) SUBNAME
FROM EMP;
#OR___USING_RIGHT()_FUNCTION
SELECT RIGHT(FNAME, 4) ALIASNAME
FROM EMP;
...
#OR
SELECT * FROM EMP
WHERE LEFT(FNAME,1) IN ('A','E','I','O','U');
...
FROM EMP;
#OR
SELECT LEFT(FNAME, (length(FNAME) / 2))
FROM EMP;


#104.
SELECT SUBSTR(FNAME, (length(FNAME) / 2))
FROM EMP;
#OR
SELECT RIGHT(FNAME, (length(FNAME) / 2))
FROM EMP;

#EXAMPLE_OF_INTERVIEW_QUESTION
SELECT FNAME, concat(SUBSTR(FNAME,1,3) , LENGTH(CONCAT(FNAME,LNAME)), SUBSTR(LNAME,1,3)) EMP_PASSWORD
FROM EMP
ORDER BY FNAME DESC;
#OR
SELECT FNAME, concat(LEFT(FNAME,3) , LENGTH(CONCAT(FNAME,LNAME)), RIGHT(LNAME,3)) EMP_PASSWORD
FROM EMP
ORDER BY FNAME DESC;


#105
SELECT concat(UPPER(LEFT(FNAME, (length(FNAME) / 2))), LOWER(RIGHT(FNAME,(LENGTH(FNAME)/2)))) UPPER_LOWER
FROM EMP;
#AND_USING_REVERSE_INCASE_OF_FIRSTeman
SELECT concat(UPPER(SUBSTR(FNAME, 1,(length(FNAME) / 2))), LOWER(REVERSE(SUBSTR(FNAME,(LENGTH(FNAME)/2+1))))) UPPER_LOWER
FROM EMP;

#106_Requirment
SELECT length('MALAYALAM') - length(REPLACE('MALAYALAM', 'A', '')) LENG_OF_A;

#107
SELECT FNAME, LNAME
FROM EMP
WHERE LENGTH(FNAME)-length(REPLACE(UPPER(FNAME),"H",'')) =1;
#OR
SELECT FNAME, LNAME
FROM EMP
WHERE length(REPLACE(UPPER(FNAME),"H",'')) = length(FNAME)-1;

#108
SELECT FNAME, LNAME
FROM EMP
WHERE LENGTH(FNAME)-length(REPLACE(UPPER(FNAME),"A",'')) =2;
#OR
SELECT FNAME, LNAME
FROM EMP
WHERE length(REPLACE(UPPER(FNAME),"A",'')) = length(FNAME)-2;
